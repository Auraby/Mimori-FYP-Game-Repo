...
//			for (int i = 0; i < rendererObjects.Length; i++)
//			{
//				rendererObjects [i].gameObject.SetActive (false);
//			}
//		}
//
//
...
//	}
	...
	void Start(){
		...
		tempTransparency = this.gameObject.GetComponent<Renderer> ().material.GetFloat ("_Transparency");
	}

	void Update(){
		transparency = this.gameObject.GetComponent<Renderer> ().material.GetFloat ("_Transparency");
		if (interacted && transparency > 0) {
			tempTransparency -= Time.deltaTime /4;
			this.gameObject.GetComponent<Renderer> ().material.SetFloat ("_Transparency", tempTransparency);
		}
